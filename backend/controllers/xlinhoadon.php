...
if (isset($_GET['donhang'])) {
    ...
    if (mysqli_num_rows($result) > 0) {
        ...
        $data = "
            ...
            <button class='float-right print-pdf'>Xem PDF</button>
                    <div class='wrapper-hoadon'>
                        ...
                        <div>Địa chỉ: <h6 class='d-inline'>99 An Dương Vương, P16, Q8, TPHCM</h6></div>
        ...
        $data .= "<tr><td colspan='5'>Tổng hiền hàng hóa: <h6 class='d-inline'>" . formatCurrency($tongtien) . "</h6> </td>
            ...
            <div class='d-flex justify-content-around mt-3 mb-5'>
                <div> <h6 class='text-cent'>Người mua hàng </h6>
                    <p class='py-2 mb-5 fw-lighter'>(Ký, ghi rõ họ tên)</p>
                </div>
                <div><h6 class='text-cent'>Người bán hàng</h6>
                    <p class='py-2 mb-5 fw-lighter'>(Ký, ghi rõ họ tên)</p>
                </div>
            </div>
    ...
    $row = mysqli_fetch_assoc($result);
    if (mysqli_num_rows($result) > 0) {
        $arr = explode('-', $row['ngayLap']);
        $data = "
            <div class='btn-loaddon'>Đơn hàng > Hóa đơn</div> 
            <button class='float-right print-pdf'>Xem PDF</button>
                    <div class='wrapper-hoadon'>
                        <h2 class='text-cent'>PHIẾU NHẬP KHO</h2>
                        <p class='text-cent'>Ngày " . $arr[2] . " tháng " . $arr[1] . " năm " . $arr[0] . "</p>
                        <span></span><h5 class='d-inline'> Cửa hàng mỹ phẩm Belleza</h5><br>
                        <span >Mã số thuế:<h6 class='d-inline'> 0123456789</h6></span>
                        <div>Địa chỉ: <h6 class='d-inline'>99 An Dương Vương, P16, Q8, TPHCM</h6></div>
                        <div class='horizontal-line'></div>

                        <span>Nhà cung cấp: <h6 class='d-inline'>". $row['TenNCC']."</h6></span><br>
                        <span>Địa chỉ: <h6 class='d-inline'>". $row['DiaChi']."</h6> </span><br>
                        <div>Số điện thoại: <h6 class='d-inline'>".$row['SoDienThoai']."</h6></div><br>
                        <table class='table table-bordered mt-2' style='border: 2px solid black'>
                            <thead>
                            <tr>
                                <th>STT</th>
        ...
        $data .= "<tr><td colspan='5'>Tổng tiền hàng hóa: <h6 class='d-inline'>" . formatCurrency($tongtien) . "</h6> </td>
            </tr></tbody>
            </table>
            <span>Số tiền viết bằng chữ:<h6 class='d-inline text-capitalize'> " . $transformer->toCurrency($tongtien) . "</h6></span>
            <div class='d-flex justify-content-around mt-3 mb-5'>
                <div> <h6 class='d-inline'>Người giao hàng </h6>
                    <p class='py-2 mb-5 fw-lighter'>(Ký, ghi rõ họ tên)</p>
                </div>
                <div><h6 class='text-cent'> Thủ kho </h6>
                    <p class='py-2 mb-5 fw-lighter'>(Ký, ghi rõ họ tên)</p>
                </div>
            </div>
            <footer style='text-align: center;font-size:12px;'>
